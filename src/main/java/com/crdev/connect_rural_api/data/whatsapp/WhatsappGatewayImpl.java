package com.crdev.connect_rural_api.data.whatsapp;

import com.crdev.connect_rural_api.app.whatsapp.dto.SendWhatsappMessageRequest;
import com.crdev.connect_rural_api.business.whatsapp.WhatsappGateway;
...
public class WhatsappGatewayImpl implements WhatsappGateway {
    ...
    public void sendTextMessage(UUID appKey, String to, String text) {
        send(appKey, SendWhatsappMessageRequest.builder()
                .to(to)
                .type("TEXT")
                .text(text)
                .build());
    ...
    @Override
    public void send(UUID appKey, SendWhatsappMessageRequest request) {
        try {
            whatsappGatewayClient.post()
                    ...
                    .retrieve()
                    .toBodilessEntity();
            log.info("[WhatsApp] Mensaje {} enviado a {} vía gateway", request.getType(), request.getTo());
        } catch (Exception e) {
            log.error("[WhatsApp] Error al enviar mensaje a {} vía gateway: {}", request.getTo(), e.getMessage());
