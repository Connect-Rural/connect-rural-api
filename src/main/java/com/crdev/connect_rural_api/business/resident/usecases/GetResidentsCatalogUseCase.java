...
@Component
@AllArgsConstructor
public class GetResidentsCatalogUseCase {
    private final ResidentService residentService;
    private final CommunityService communityService;
    private final ResidentAppMapper mapper;

    @Transactional
    public List<SimpleResidentResponseDto> execute(String communityKey) {
        CommunityEntity community = communityService.getBykey(communityKey);
        return residentService
                .getResidentsCatalogByCommunityKey(community.getKey().toString())
                .stream()
